...
public class InquiryProductRecordUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InquiryProductRecordUI frame = new InquiryProductRecordUI();
	...
	public InquiryProductRecordUI() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				ProductRecord[] pdrList = null;
				
				try {
					pdrList = (ProductRecord[])(rm.inquiryRecordList(RecordManager.PRODUCT_RECORD, t1, t2));
					...
					for(int i=0; i<pdrList.length; i++) {
						String mngID = null;
						if(pdrList[i].getManagementID() == 0) mngID = "등록";
						if(pdrList[i].getManagementID() == 1) mngID = "수정";
						if(pdrList[i].getManagementID() == 2) mngID = "삭제";
						
						model.addRow(new Object[]{
							mngID,
							pdrList[i].getProductID(),
							pdrList[i].getProductName(),
							new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(pdrList[i].getRecordTime())});
					}
