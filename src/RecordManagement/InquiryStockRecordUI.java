...
public class InquiryStockRecordUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InquiryStockRecordUI frame = new InquiryStockRecordUI();
	...
	public InquiryStockRecordUI() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				StockRecord[] strList = null;
				
				try {
					strList = (StockRecord[])(rm.inquiryRecordList(RecordManager.STOCK_RECORD, t1, t2));
					...
					for(int i=0; i<strList.length; i++) {
						String mngID = null;
						if(strList[i].getManagementID() == 0) mngID = "등록";
						if(strList[i].getManagementID() == 1) mngID = "수정";
						if(strList[i].getManagementID() == 2) mngID = "삭제";
						
						model.addRow(new Object[]{
							mngID,
							strList[i].getProductName(),
							new SimpleDateFormat("yyyy-MM-dd").format(strList[i].getExpirationDate()),
							new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(strList[i].getRecordTime())});
					}
