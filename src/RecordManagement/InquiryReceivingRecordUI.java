...
public class InquiryReceivingRecordUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InquiryReceivingRecordUI frame = new InquiryReceivingRecordUI();
	 ...
	 * Create the frame.
	 */
	public InquiryReceivingRecordUI() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				ReceivingRecord[] rcrList = null;
				
				try {
					rcrList = (ReceivingRecord[])(rm.inquiryRecordList(RecordManager.RECEIVING_RECORD, t1, t2));
					...
					for(int i=0; i<rcrList.length; i++) {
						String mngID = null;
						if(rcrList[i].getManagementID() == 0) mngID = "등록";
						if(rcrList[i].getManagementID() == 1) mngID = "수정";
						if(rcrList[i].getManagementID() == 2) mngID = "삭제";
						
						model.addRow(new Object[]{
							mngID,
							rcrList[i].getProductName(),
							rcrList[i].getQuantity(),
							new SimpleDateFormat("yyyy-MM-dd").format(rcrList[i].getReceivingDate()),
							new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(rcrList[i].getRecordTime())});
					}
