...
public class InquiryReturnRecordUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InquiryReturnRecordUI frame = new InquiryReturnRecordUI();
	...
	public InquiryReturnRecordUI() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				ReturnRecord[] rtrList = null;
				
				try {
					rtrList = (ReturnRecord[])(rm.inquiryRecordList(RecordManager.RETURN_RECORD, t1, t2));
					...
					for(int i=0; i<rtrList.length; i++) {
						String mngID = null;
						if(rtrList[i].getManagementID() == 0) mngID = "등록";
						if(rtrList[i].getManagementID() == 1) mngID = "수정";
						if(rtrList[i].getManagementID() == 2) mngID = "삭제";
						
						model.addRow(new Object[]{
							mngID,
							rtrList[i].getProductName(),
							rtrList[i].getSupplierName(),
							new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(rtrList[i].getRecordTime())});
					}
