...
public class InquirySaleRecordUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InquirySaleRecordUI frame = new InquirySaleRecordUI();
	...
	public InquirySaleRecordUI() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				SaleRecord[] slrList = null;
				
				try {
					slrList = (SaleRecord[])(rm.inquiryRecordList(RecordManager.SALE_RECORD, t1, t2));
					...
					for(int i=0; i<slrList.length; i++) {
						String mngID = null;
						if(slrList[i].getManagementID() == 0) mngID = "등록";
						if(slrList[i].getManagementID() == 1) mngID = "수정";
						if(slrList[i].getManagementID() == 2) mngID = "삭제";
						
						model.addRow(new Object[]{
							mngID,
							slrList[i].getProductName(),
							slrList[i].getQuantity(),
							new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(slrList[i].getRecordTime())});
					}
